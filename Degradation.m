    ...
    for i = 1:length(rfCountingply{k})
        ...
        if R02<=rfCountingply{k}(i,4) && rfCountingply{k}(i,4)<R01
            for j = 1:floor(UTS)
                ...
                amp1(j) = UTS*(1-R01)/2/((NoCycle2(j)*c1-c1+1)^s1);%amp1 which have the same nember of cycles yo failure.
            ...
            if all(in(:)==0)
                ...
                amplitude = amp2(1)*rfCountingply{k}(i,2)/2/y_intersect+amp2(k);
                rfCountingply{k}(i,5) = (1/c2)*(c2-1+(UTS/(2*amplitude/(1-R02)))^(1/s2));
            end
        end
        if R02>rfCountingply{k}(i,4) || rfCountingply{k}(i,4)>R03
            for j = 1:floor(UTS)
                ...
                mean2(j) = 0;
                NoCycle2(j) = (1/c2)*(c2-1+(UTS/(j*2/(1-R02)))^(1/s2));
                amp3(j) = UCS*(1-R03)/2/((NoCycle2(j)*c3-c3+1)^s3);%amp1 which have the same nember of cycles yo failure.
                mean3(j) = (1+R01)/(1-R01)*amp3(j);
            end
            for j = 1:floor(UTS)-1
...
for year = 1:200
    ...
    for k = 1:length(rfCountingply)
        for i = 1:length(rfCountingply{k})
            ...
            if abs(abs(rfCountingply{k}(i,2)/(1-rfCountingply{k}(i,4)))-rfCountingply{k}(i,8))<1e-6 || abs(abs(rfCountingply{k}(i,2)/(1-rfCountingply{k}(i,4)))-rfCountingply{k}(i,9))<1e-6
                fprintf('FAIL\n');
                return
            end
        end
